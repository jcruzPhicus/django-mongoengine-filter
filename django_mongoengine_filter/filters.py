...
def _truncate(dt): return dt.replace(hour=0, minute=0, second=0)  # noqa
...
class DateRangeFilter(ChoiceFilter):
    options = {
        ...
        4: (
            _("This year"),
            lambda qs, name: qs.filter(**{"%s__year" % name: now().year}),
        ),
    }
...
class MethodFilter(Filter):
    """
    This filter will allow you to run a method that exists on the
    filterset class
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if type(action) in text_types:
            ...
            kwargs.update({"action": self.filter})
    ...
    def filter(self, qs, value):
        """
        This filter method will act as a proxy for the actual method we want
        to call. It will try to find the method on the parent filterset, if
        not it defaults to just returning the queryset
        """
